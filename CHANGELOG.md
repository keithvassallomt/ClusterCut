...
- `Message::Clipboard` payload wire format changed from JSON `Vec<u8>` int-array (≈3.57× inflation for random encrypted bytes) to base64 string (≈1.33× inflation). Pre-fix, encrypted blobs over ~13 MB silently exceeded the receiver's 64 MB transport cap and dropped — manifested most visibly when copying multi-MB JPEG photos. With the descriptor path now handling > 10 MB images, the inline path stays well clear of the transport cap. **Wire-format break against pre-0.3.0 peers** (already incompatible due to pairing-protocol security work in this release).
...
- Windows firewall rule now also covers outbound UDP/4654 explicitly. The 0.2.x rule was inbound-only, so on Defender / enterprise / "Block all outbound" private-profile configs Windows would accept incoming QUIC packets but fail to send the handshake reply — surfacing as "peer-A → Windows fails, Windows → peer-A works." The rule also gains a `remoteip=any` scope and a versioned description sentinel; existing 0.2.x rules will trigger one UAC prompt on next launch to widen them.
