...
- Paste-Special → HTML in Word on a Windows receiver no longer shows only the last few characters of a copy from PyCharm (or any other app that emits a full `<html><body>…</body></html>` document on the wire). `clipboard-win` wraps the input in its own `<html><body><!--StartFragment-->…<!--EndFragment--></body></html>` envelope, so a full-document input produced nested `<html>/<body>` tags and Word's HTML importer recovered onto the tail-end fragment. CF_HTML now strips down to body content before being wrapped. CF_RTF and CF_UNICODETEXT were never affected (Notepad / VSCode / Paste-Special-as-RTF were already fine).

### Changed
- Receiving a rich-text payload on a GNOME machine now lands the **plain text** on the clipboard by default and surfaces a one-click "Switch to rich format" banner. Mutter's `Meta.SelectionSource` is single-MIME and can't be subclassed for multi-MIME from GJS — see [GJS #255](https://gitlab.gnome.org/GNOME/gjs/-/issues/255), [gnome-shell-extension-clipboard-indicator](https://github.com/Tudmotu/gnome-shell-extension-clipboard-indicator) (most-deployed reference, also single-MIME). The extension's `_writeFormats` is therefore last-write-wins, which previously meant only `text/rtf` was advertised after receive — pastes into gedit, GNOME Text Editor, OnlyOffice and browser inputs got nothing. Plain-text-first preserves the common case; the banner promotes to the rich payload on demand and the `IGNORED_CONTENT` lenient subset rule keeps the rewrite from echoing back to the sender. Other receivers (Windows, macOS, wlroots) write all MIMEs atomically and are unaffected.
