...
- After a device leaves and re-appears under its new cluster name, already-running peers now see the new cluster without needing an app restart. The mDNS service instance is keyed on the device id, so re-advertising under a new cluster name only changed a TXT record — and browsers that already had the device cached never re-resolved it, leaving the new cluster invisible until they restarted. A device now takes a fresh device id when it leaves/resets, so it re-appears as a brand-new service that every peer resolves cleanly. (The mDNS de-registration is also now flushed before re-registering.)
