...
- After a device leaves and re-appears under its new cluster name, already-running peers now see the new cluster without needing an app restart. Re-advertising over mDNS unregistered and immediately re-registered the same instance, so the "goodbye" often never went out and peers kept the stale cached name; the re-register now waits for the removal to be sent first, so peers cleanly re-resolve the new name.
