# Wire Protocol 0.3.2 — Key-Confirmation Hardening Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Close the SPAKE-pairing online brute-force budget-bypass by inserting an explicit initiator key-confirmation packet before the responder reveals its AEAD-encrypted identity; hard-break the wire by bumping `proto` to `0.3.2`; and tighten the existing version-mismatch warning system so out-of-date peers are visibly flagged before users attempt to pair.

**Architecture:** Insert a new T2 `InitiatorKC { nonce, ciphertext }` frame into the pairing flow — an AEAD-encrypted fixed-label payload encrypted under `k_i2r` (the existing initiator→responder sub-key derived from the SPAKE2 transcript). The responder must decrypt it cleanly *before* sending its own AEAD-encrypted identity; any tag failure trips the existing `record_pairing_aead_failure` counter that already gates the H1 lockout. Bump `CLUSTERCUT_PROTOCOL_VERSION` to `"0.3.2"`; raise the backend (`is_protocol_compatible`) and frontend (`MIN_COMPATIBLE_PROTOCOL`) floors in lockstep — the frontend floor currently lags backend by one minor, which we fix here. Add a pre-flight version check in `start_pairing` so discovered LAN peers below floor surface in the existing "Peer needs updating" modal *before* a TCP socket opens, instead of falling out as a generic pairing failure.

**Tech Stack:** Rust + Tauri (backend), React + TypeScript (frontend). No new dependencies.

---

## Wire Protocol 0.3.2 Reference

This plan is the sole spec for 0.3.2. Diff from 0.3.1:
...
**Pre-0.3.2 attack:** attacker initiates T0, receives T1 + the responder's encrypted T2 (`ResponderId`, ciphertext under `k_r2i` derived from the *attacker's* guessed PIN). He disconnects without sending T3. The responder logs no AEAD failure (the failure was that he didn't send T3 — but we only count AEAD-tag failures), so no counter increment. The attacker brute-forces his guessed PIN against the captured `ResponderId` ciphertext offline, learning yes/no for one PIN value per cycle. Repeat unbounded.

**Post-0.3.2 defence:** the responder won't send `ResponderId` until after it has verified `InitiatorKC` under `k_i2r`. The attacker's options:
...
Either way the attacker's budget is bounded. Importantly: wrong-variant frames do *not* trip the counter, so honest 0.3.1 clients hitting a 0.3.2 responder don't lock the listener out for the user.

### Compatibility

This is a hard wire break. A 0.3.2 responder receiving a 0.3.1 initiator's `InitiatorId` at the T2 slot sees a wrong-variant frame; `log_pairing_failure` fires and the connection closes (no lockout counter bump). A 0.3.2 initiator hitting a 0.3.1 responder will send `InitiatorKC` at T2, which the older responder reads where it expects `InitiatorId`, also a wrong-variant log + close. The pre-flight check in Task 8 catches LAN cases with mDNS `proto` data and surfaces the existing "Peer needs updating" modal before opening the socket — for Add-Remote (no mDNS), wire-level failure with a generic message remains the only signal.
...
**Modify:**
- [src-tauri/src/discovery.rs](src-tauri/src/discovery.rs) — bump `CLUSTERCUT_PROTOCOL_VERSION` to `"0.3.2"`; extend the doc block.
...
- [src/App.tsx](src/App.tsx) — bump `MIN_COMPATIBLE_PROTOCOL` to `[0, 3, 2]`.
...
## Task 1: Bump `CLUSTERCUT_PROTOCOL_VERSION` to `"0.3.2"`

**Files:**
- Modify: [src-tauri/src/discovery.rs:7-17](src-tauri/src/discovery.rs#L7-L17)

The mDNS `proto` TXT property is the load-bearing version signal. Bumping it does two things: 0.3.1 peers see the 0.3.2 advertiser as future-version and flag it (their amber-triangle code matches on exact version delta), and 0.3.2 peers using the new `is_protocol_compatible` floor refuse the older peer at the per-peer send path.
...
git commit -m "Bump CLUSTERCUT_PROTOCOL_VERSION to 0.3.2 (wire break)"
```
...
The KC plaintext is a fixed byte string. The AEAD tag computed over `(k_i2r, nonce, KC_PLAINTEXT)` is what authenticates the initiator's key derivation; the plaintext itself carries no information. A versioned label binds the constant to 0.3.2 so a future revision can't accidentally accept the older form.
...
git commit -m "Add InitiatorKC variant to PairingMessage (wire 0.3.2)"
```
...
Run: `cd src-tauri && cargo build 2>&1 | tail -20`
Expected: `Finished` with no errors. Initiator still sends old T3 (`InitiatorId`) so two 0.3.2 builds talking to each other still fail — Task 5 fixes the initiator.

- [ ] **Step 6: Commit**

```bash
git add src-tauri/src/lib.rs
git commit -m "Responder: require InitiatorKC at T2 before sending ResponderId (wire 0.3.2)"
```
...
- [ ] **Step 6: Manual end-to-end check (two 0.3.2 builds)**
...
- 0.3.2 initiator → 0.3.1 responder: pairing fails with a generic message (the 0.3.1 responder is reading what it thinks is `InitiatorId` at the old T3 slot but gets `InitiatorKC`; it logs and closes).
- 0.3.1 initiator → 0.3.2 responder: pairing fails. 0.3.2 responder reads `InitiatorId` at T2, logs wrong-variant (no counter bump), closes.

- [ ] **Step 7: Commit**

```bash
git add src-tauri/src/lib.rs
git commit -m "Initiator: send InitiatorKC at T2 before reading ResponderId (wire 0.3.2)"
```

---

## Task 6: Raise backend `is_protocol_compatible` floor to `(0, 3, 2)`
...
If you have a 0.3.1 (or earlier) peer on the LAN, confirm in the running 0.3.2 build that:
...
```bash
git add src/App.tsx
git commit -m "Raise frontend MIN_COMPATIBLE_PROTOCOL to 0.3.2"
```
...
With a 0.3.1 (or earlier) peer on the LAN and the running 0.3.2 build:
...
### Changed
- Wire protocol bumped to 0.3.2. Hard break — 0.3.1 peers will surface in the existing "please upgrade" UI flow. Frontend version-mismatch indicator now correctly reflects the backend floor (was stale at 0.3.0).
```
...
git commit -m "Changelog: wire 0.3.2 key-confirmation hardening"
```
...
- Warning system: amber triangle correctly fires for sub-0.3.2 peers — Task 7 (fix stale frontend floor). ✓
...
Plan complete and saved to `docs/superpowers/plans/2026-05-27-wire-protocol-0.3.2.md`. Two execution options:
